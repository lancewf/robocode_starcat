...
object ForwardTurretOrientationObserverCodeletActor{
  def getProps(
    successActivationRecipients: List[SlipnetNodeActorActivationRecipient]): Props = {
    Props(classOf[ForwardTurretOrientationObserverCodeletActor], successActivationRecipients)
  }
}
/**
 * 
 ...
 *          West   North  East
 */ 
class ForwardTurretOrientationObserverCodeletActor(
  ...
  protected def cloneCodelet():ActorRef ={
    context.system.actorOf(ForwardTurretOrientationObserverCodeletActor.getProps(
